// internal/service/song_service_test.go
...
import (
	...
	mock_musicapi "songlibrary/internal/musicapi/mocks" // Путь к mock файлам musicapi
	"songlibrary/internal/service"
	"songlibrary/internal/storage"
	mock_storage "songlibrary/internal/storage/mocks" // Путь к mock файлам storage
...
func TestSongService_AddSong(t *testing.T) {
	testCases := []struct {
		...
		mockMusicAPIFn func(m *mock_musicapi.MockMusicAPIClient)
		expectError    bool
	}{
		{
			name: "Valid request",
			...
			mockStorageFn: func(s *mock_storage.MockSongStorage) {
				s.EXPECT().Create(gomock.Any(), gomock.Any()).Return(&models.Song{ID: 1, GroupName: "Test Group", SongName: "Test Song"}, nil)
			},
			mockMusicAPIFn: func(m *mock_musicapi.MockMusicAPIClient) {
		...
		},
		{
			...
			mockStorageFn: func(s *mock_storage.MockSongStorage) {
				s.EXPECT().Create(gomock.Any(), gomock.Any()).Return(nil, errors.New("storage error"))
			},
			mockMusicAPIFn: func(m *mock_musicapi.MockMusicAPIClient) {
				m.EXPECT().GetSongDetailsFromAPI("Test Group", "Test Song").Return(&models.SongDetailFromAPI{Text: "Test Text", ReleaseDate: "2023-01-01", Link: "http://test.link"}, nil)
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockMusicAPIClient := mock_musicapi.NewMockMusicAPIClient(ctrl)
...
func TestSongService_GetSongs(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			serviceInstance := service.NewSongService(mockStorage, nil) // musicAPIClient is not used in GetSongs
...
func TestSongService_GetSongText(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			songID: 1,
			mockStorageFn: func(s *mock_storage.MockSongStorage) {
				s.EXPECT().GetByID(gomock.Any(), 1).Return(&models.Song{ID: 1, GroupName: "Test Group", SongName: "Test Song", Text: stringPointer("Test Text")}, nil)
			},
			expectError:  false,
			expectedSong: &models.Song{ID: 1, GroupName: "Test Group", SongName: "Test Song", Text: stringPointer("Test Text")},
		},
		{
			name:   "Song not found",
			songID: 1,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			serviceInstance := service.NewSongService(mockStorage, nil) // musicAPIClient is not used in GetSongText

			song, err := serviceInstance.GetSongText(context.Background(), tc.songID)

			if tc.expectError {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
				assert.Equal(t, tc.expectedSong, song)
			}
		})
	}
}
...
func TestSongService_UpdateSong(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			serviceInstance := service.NewSongService(mockStorage, nil) // musicAPIClient is not used in UpdateSong
...
func TestSongService_DeleteSong(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			serviceInstance := service.NewSongService(mockStorage, nil) // musicAPIClient is not used in DeleteSong
